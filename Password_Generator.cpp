...
/* selectArray is utility function that is used to
 * randomly generate a integer in a range 1 to 4 (both inclusive) */
...
{
	// Initial result string password as NULL.
	string password = "";
	...
	while (count < length)
	{
		 ...
		 * ALPHABET (i.e 1 or 2)
		 * following if conditions will take care of it. */
		if (count == 0)
		{
			k = k % 3;
			if (k == 0)
		...
		case 1:
			 ...
			 * characters is still left than it will break */
			if ((count_alphabet == 2) && (count_number == 0 || count_ALPHABET == 1 || count_s_symbol == 0))
				break;
		...
		case 2:
			 ...
			 * ALPHABET chareacter has been fulfilled or not
			 * in case it has been fulfilled and minimum requirement of
		...
		case 3:
			 ...
			 * other characters is still left than it will break */
		...
		case 4:
			/* Following if condition will check if minimum requirement of
			 * special symbol character has been fulfilledor not
			 * in case it has been fulfilled and minimum requirements of
			 * other characters is still left than it will break */
...
int main()
{
	...
	do
	{
		...
		case 1:
			...
			// Length should not exceed 100, program should show if it exceeds
			else if (length > 100)
			{
		...
		default:
			// If invalid option is choose by user it will also show error
			if (opt != 2)
			{
				cout << "\nInvalid choice\n";
